      ...
      // Assuming DataManager is in DB namespace
        ...
        private void DisplayPage(int pageNumber)
        {
            ...
            var rowsToRemove = tablePanel.Controls.OfType<Panel>()
                                       .Where(p => p.Tag?.ToString() == "DoctorRow")
                                       .ToList();
            ...
            int yPos = tablePanel.Controls.OfType<Panel>().FirstOrDefault(p => p.Dock == DockStyle.Top)?.Bottom ?? 0; // Start below header
            foreach (var doctor in doctorsToShow)
            {
                string name = doctor.Name ?? "N/A";
                string specialization = doctor.Specialization ?? "N/A";
                string workload = doctor.Workload.ToString() ?? "0";

                AddDoctorRow(tablePanel, 0, yPos, name, specialization, workload, doctor.Id);
        ...
        private void AddDoctorRow(Panel parent, int x, int y, string name, string specialty, string patientCount, int doctorId)
        {
            Panel rowPanel = new Panel
            {
                ...
                Tag = "DoctorRow", // Tag to identify for removal
            ...
            // Bring header to front to ensure it overlays rows if docking rows is used later
            parent.Controls.OfType<Panel>().FirstOrDefault(p => p.Dock == DockStyle.Top)?.BringToFront();
        }
        ...
        private void InitializeComponentDoctors()
        {
            ...
            // --- Title --- (Positioned manually within contentPanel's padding)
            Label doctorsTitle = new Label
            {
                Text = "Doctors",
                ForeColor = ColorTranslator.FromHtml("#2C3E50"), // Dark blue-gray text
                Font = new Font("Segoe UI", 24, FontStyle.Bold),
                Location = new Point(0, 0), // Position at top-left of contentPanel padding area
                AutoSize = true,
                Anchor = AnchorStyles.Top | AnchorStyles.Left // Anchor to top-left
            };
            contentPanel.Controls.Add(doctorsTitle); // Add to contentPanel
            ...
            tablePanel = new Panel
            {
                ...
                Dock = DockStyle.Fill // DOCK TO FILL remaining space
            };
            // Add Table Header Panel first (Dock=Top within tablePanel)
            InitializeTableHeader(tablePanel);
            contentPanel.Controls.Add(tablePanel); // Add to contentPanel


            // Set Tab Order / BringToFront if needed (Docking order usually handles layers)
            searchPanel.BringToFront(); // Ensure search is above table visually
            doctorsTitle.BringToFront(); // Ensure title is above search panel
        ...
        private void InitializeSearchPanelControls(Panel parent)
        {
            ...
            string[] specialties = { "All Specialties", "Cardiology", "Neurology", "Orthopedics", "Pediatrics", "Oncology" }; // Example
        ...
        private void InitializePaginationControls(Panel parent)
        {
            ...
            int maxTotalButtons = 7; // e.g., Prev + 5 numbers + Next
        ...
        // Helper to setup the table header
        private void InitializeTableHeader(Panel parentTablePanel)
        {
            Panel tableHeaderPanel = new Panel
            {
                Height = 40,
                BackColor = ColorTranslator.FromHtml("#F8F9FA"), // Very light gray
                Dock = DockStyle.Top, // Dock header to top of tablePanel
                Padding = new Padding(20, 0, 20, 0) // Horizontal padding
            };
            parentTablePanel.Controls.Add(tableHeaderPanel); // Add header TO tablePanel

            // Define column headers
            string[] columnHeaders = { "Name", "Specialty", "Patients", "Actions" }; // Length = 4

            // Define approximate start X positions for EACH header. Must match columnHeaders length.
            int[] columnStartX = {
                 tableHeaderPanel.Padding.Left,                // Name starts at left padding (Index 0)
                 ...
                 tableHeaderPanel.Padding.Left + 220 + 220 + 170 // <<<--- ADDED A VALUE FOR ACTIONS (Index 3)
                                                                 // (This value provides a valid array element. The final position
                                                                 // for "Actions" might be overridden below by right-anchoring logic,
                                                                 // but this prevents the IndexOutOfRangeException during initial access)
            };

            // *** Good Practice: Add a check to ensure arrays match ***
            if (columnStartX.Length != columnHeaders.Length)
            {
                // Log the error or throw a more specific exception during development
                AddLogMessage($"CRITICAL ERROR: Mismatch between columnHeaders ({columnHeaders.Length}) and columnStartX ({columnStartX.Length}) array lengths in InitializeTableHeader.");
                // Optionally prevent further execution in this method:
                // throw new InvalidOperationException("Header definition arrays do not match in length.");
                return; // Exit the method gracefully if possible
            }


            for (int i = 0; i < columnHeaders.Length; i++) // Loop runs 0, 1, 2, 3
            {
                Label headerLabel = new Label
                {
                    Text = columnHeaders[i], // Accesses [0]..[3] - OK
                    ForeColor = ColorTranslator.FromHtml("#2C3E50"),
                    Font = new Font("Segoe UI", 11, FontStyle.Bold),
                    // Accessing columnStartX[i] is now SAFE for i=0, 1, 2, 3
                    Location = new Point(columnStartX[i], (tableHeaderPanel.ClientSize.Height - 18) / 2), // Vertical center
                    AutoSize = true
                };

                // Special positioning for 'Actions' column to align it better with buttons below
                if (columnHeaders[i] == "Actions")
                {
                    // This logic overrides the Location.X set above for the "Actions" label
                    // Position it relative to the right edge for better alignment with action buttons in rows
                    int approxActionButtonsWidth = 100; // Estimated space needed for E/D buttons + padding
                    headerLabel.Location = new Point(tableHeaderPanel.ClientSize.Width - tableHeaderPanel.Padding.Right - approxActionButtonsWidth, headerLabel.Top); // Position from Right
                    headerLabel.Anchor = AnchorStyles.Top | AnchorStyles.Right; // Anchor it right
                }
                else
                {
                    headerLabel.Anchor = AnchorStyles.Top | AnchorStyles.Left; // Anchor others left
                }
        ...
        // Auto-generated Load method (can be used instead of constructor for loading if preferred)
        private void DoctorsForm_Load(object sender, EventArgs e)
        {
            // Data is loaded in constructor in this version
            // if (!this.DesignMode) {
            //      // LoadDoctorsData();
            // }
        }
